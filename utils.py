...
class myAugmentation(object):
    ...
    """
    def __init__(self, train_path=config.train_imgs_path, label_path=config.train_labels_path,
                 merge_path=config.merge_path, aug_merge_path=config.aug_merge_path,
                 aug_train_path=config.aug_train_path, aug_label_path=config.aug_label_path, img_type=config.img_type):
        """
    ...
    def doAugmentate(self, img, save_to_dir, save_prefix, batch_size=1,
                     save_format=config.img_type, imgnum=config.aug_img_num):
        # 增强一张图片的方法
        """
    ...
    def splitTransform(self):
        ...
        path_merge = config.aug_merge_path
        path_train = config.aug_train_path
        path_label = config.aug_label_path
...
class dataProcess(object):
    def __init__(self, out_rows, out_cols, data_path = config.train_imgs_path,
                 label_path = config.train_labels_path, test_path = config.test_path,
                 npy_path = config.npy_path, img_type = config.img_type):
        # 数据处理类，初始化
...
# 以下注释掉的部分为数据增强代码，通过他们可以将数据进行增强
    ...
    mydata = dataProcess(config.norm_size,config.norm_size)
